...
class IRSystem:
	...
	def compute_tfidf(self):
		...
		for d in range(len(self.titles)):
			for word in self.vocab:
				if self.pos_index[tuple([word, d])] != 0 and self.inv_index[word] != 0:
					tf = len(self.pos_index[tuple([word, d])]) # number of times word appears in doc d
					df = len(self.inv_index[word]) # number of docs that contain t
					if word not in self.tfidf:
						self.tfidf[word] = {}
					self.tfidf[word][d] = 1 + math.log10(tf)
					self.tfidf[word][d] *= (math.log10(n) - math.log10(df))
	...
	def index(self):
		...
		# If you have |V| words in your vocab, do you really need to go through each word in each document |V| times?  

		inv_index = collections.defaultdict(lambda: 0) 
		pos_index = collections.defaultdict(lambda: 0)
		for docId in xrange(0, len(self.docs)):
			temp = self.docs[docId][:]
			for token in xrange(0, len(temp)):
				if inv_index[temp[token]] == 0:
					inv_index[temp[token]] = set()
				(inv_index[temp[token]]).add(docId)
				if pos_index[tuple([temp[token], docId])] == 0:
					pos_index[tuple([temp[token], docId])] = []
				(pos_index[tuple([temp[token], docId])]).append(token)
	...
	def boolean_retrieve(self, query):
		...
		intersection = set()
		if len(query) >= 1:
			intersection = self.inv_index[query[0]]
		for word in query:
			intersection = intersection & self.inv_index[word]
	...
	def phrase_retrieve(self, query):
		...
		if len(query) == 0:
			return docs
		has_words = self.boolean_retrieve(query)

		if len(query) == 1:
			return has_words


		for docId in has_words:
			# print self.pos_index[tuple([query[0]]), docId]
			base = set()
			for index in xrange(0, len(query)):
				if index == 0:
					base = set(self.pos_index[tuple([query[index], docId])])
					
				elif len(base) != 0:
					temp = [item + 1 for item in base]
					base = set(temp) & set(self.pos_index[tuple([query[index], docId])])
	...
	def rank_retrieve(self, query):
		...
		doc_vector = [collections.defaultdict(lambda: 0)  for xx in range(len(self.titles))]
		q_vector = collections.defaultdict(lambda: 0) 
		for word in words_in_query:
			q_vector[word] = 1 + math.log10(query.count(word))
		
		norm = 0.0

		for d in xrange(0, len(self.titles)):
			doc_vector[d] = {}
			for word in self.docs[d]:
				doc_vector[d][word] = self.tfidf[word][d]
				norm += math.pow(doc_vector[d][word], 2)
				
			norm = 1/math.sqrt(norm)
			for word in self.docs[d]:
				doc_vector[d][word] = float(doc_vector[d][word]) * float(norm)
				
			for word in words_in_query:
				if word in self.docs[d] and q_vector[word] != 0:
					scores[d] += q_vector[word] * doc_vector[d][word]
